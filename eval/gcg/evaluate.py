...
def parse_args():
    ...
    parser.add_argument("--split", required=True, choices=['val', 'test'], help="Evaluation split, options are 'val', 'test'")
...
def calculate_metrics(bert_model, gt_dir_path, prediction_dir_path, split):
    ...
    assert split in ["val", "test"] # GCG Evaluation has only val and test splits
