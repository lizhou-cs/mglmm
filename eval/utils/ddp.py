...
from eval.utils.utils import bbox_to_x1y1x2y2


class ImageDDP(Dataset):
    def __init__(self, image_dir, image_list=None):
        if image_list is not None:
            self.image_list = image_list
        else:
            self.image_ids = os.listdir(image_dir)
            self.image_list = [os.path.join(image_dir, image_id) for image_id in self.image_ids]
...
class CapDDP(Dataset):
    def __init__(self, annotation_file):
        self.coco = COCO(annotation_file)
        self.image_dict = self.coco.imgs
        self.ann_dict = self.coco.imgToAnns
    ...
    def __getitem__(self, idx):
        ...
        captions = [ann['caption'] for ann in self.ann_dict[image_id]]
        gt = ';'.join(captions)
        bbox = None
