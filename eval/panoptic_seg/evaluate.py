...
def default_argument_parser():
    ...
    parser.add_argument("--dataset_dir", default="", help="path to the dataset directory")
    parser.add_argument("--dataset_name", default="coco_2017_val_panoptic", 
                        choices=["coco_2017_val_panoptic"], help="dataset name you want to evaluate")
    parser.add_argument("--json_file", default="", help="path to detection json file")
    return parser


# Modified from the official panoptic api: https://github.com/cocodataset/panopticapi/blob/master/panopticapi/evaluation.py
def pq_compute_single_image(image_id, segm_gt, segm_dt, categories, ignore_label):
    pq_stat = PQStat()
    VOID = ignore_label
    OFFSET = 256 * 256 * 256
...
def main():
    ...
    for image_id in tqdm(image_ids):
        if args.dataset_name == "coco_2017_val_panoptic":
            gt_dir = os.path.join(args.dataset_dir, "cocopanoptic", "panoptic_semseg_val2017")
        ...
        pred[pred == ignore_label] = num_classes
        ...
        # pq
        pq_stat_single = pq_compute_single_image(image_id, segm_gt, segm_dt, categories, meta.ignore_label)
