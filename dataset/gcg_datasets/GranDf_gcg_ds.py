...
class GCGBaseDataset(OSSDataset):
    """
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=8000, precision="fp32",
                 ...
                 image_dir='', json_path=''):
        super(GCGBaseDataset, self).__init__()
    ...
    def _parse_annotations(self, ann_info):
        ...
        width, height = self.oss_load_img(image_path).size
    ...
    def process_data(self, data_item):
        ...
        tokens_positive = [span for span in tokens_positive if span[0] != -1 and span[1] != -1]
...
class GranDfDataset(GCGBaseDataset):
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=8000, precision="fp32",
        ...
        image_dir = os.path.join("GranDf", "GranDf_HA_images", "train")
...
class Flickr30kGCGDataset(GCGBaseDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=8000, precision="fp32",
        ...
        json_path = json_files['validation'] if validation else json_files['training']
        image_dir = os.path.join("flickr_30k", "images")
        mode = "Val" if validation else "Train"
    ...
    def _load_annotations(self, ann_file):
        ...
        self.coco = COCO(self.oss_get_file(ann_file))
...
class RefCOCOgGCGDataset(GCGBaseDataset):
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=8000, precision="fp32",
        ...
        json_path = json_files['validation'] if validation else json_files['training']
        image_dir = os.path.join("coco", "train2014")
        mode = "Val" if validation else "Train"
    ...
    def _parse_annotations(self, ann_info):
        ...
        # If any of the tokens had their end index set before their start index because of trimming, filter them
        filtered_idx = [i for i, token in enumerate(tokens_positive) if token[0] <= token[1]]
        annotations['tokens_positive'] = [tokens_positive[i] for i in filtered_idx]
        annotations['masks'] = [annotations['masks'][i] for i in filtered_idx]
        annotations['labels'] = [annotations['labels'][i] for i in filtered_idx]
