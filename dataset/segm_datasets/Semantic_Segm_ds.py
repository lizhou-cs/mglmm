...
from dataset.utils.oss_dataset import OSSDataset, oss_list_dir, oss_load_json, oss_get_file
...
def init_ade20k(dataset_dir):
    ...
    ade20k_images = oss_list_dir(ade20k_image_dir, ".jpg")
...
def init_cocostuff(dataset_dir):
    ...
    # Annotations
    cocostuff_labels_dir = os.path.join(dataset_dir, "cocostuff")
    cocostuff_labels = oss_list_dir(cocostuff_labels_dir, ".png")
    # Images are obtained from COCO 2017 images
    cocostuff_images = [label.replace(".png", ".jpg").replace("cocostuff", "coco/train2017") for
...
def init_paco_lvis(dataset_dir):
    paco_lvis_api = COCO(oss_get_file(os.path.join(dataset_dir, "vlpart/paco", "annotations", "paco_lvis_v1_train.json")))
...
def init_pascal_part(dataset_dir):
    pascal_part_api = COCO(oss_get_file(os.path.join(dataset_dir, "vlpart/pascal_part", "train.json")))
...
def init_mapillary(dataset_dir):
    ...
    mapillary_classes = [cls["readable"].lower() for cls in
                         oss_load_json(os.path.join(mapillary_path, "config_v2.0.json"))["labels"]]
    mapillary_labels_path = os.path.join(mapillary_path, "training", "v2.0", "labels")
    mapillary_labels = sorted(oss_list_dir(mapillary_labels_path, suffix=".png"))
...
class SemanticSegmDataset(OSSDataset):
    CLASSES = ('object',)
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=500 * 8 * 2 * 10,
                 ...
                 random_sampling=True, explanatory=False):
        super(SemanticSegmDataset, self).__init__()
        ...
        for ds in self.semantic_seg_ds_list:
            ...
            self.data2num[ds] = len(images)
            print(f'\033[92m----SEG-{"Val" if validation else "Train"}: Loaded SemanticSeg - {ds} dataset ----\033[0m')
            print(f'\033[92m----SEG-{"Val" if validation else "Train"}: Number of the dataset is {len(images)}----\033[0m')
    ...
    def __getitem__(self, idx):
        ...
        if dataset_name in ["paco_lvis", "pascal_part"]:
            ...
            idx = idx if (self.validation or not self.random_sampling) else (random.randint(0, len(img_ids) - 1))
            img_info = coco_api.loadImgs([img_ids[idx]])[0]
            file_name = img_info["file_name"]
            image_path = os.path.join(
                 self.dataset_dir, 'vlpart', dataset_name, "VOCdevkit", "VOC2010", "JPEGImages", file_name
            ...
            label = np.array(self.oss_load_img(label_path))
