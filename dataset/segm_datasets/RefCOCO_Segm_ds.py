...
class ReferSegmDataset(OSSDataset):
    CLASSES = ('object',)
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=500 * 8 * 2 * 10,
                 ...
                 random_sampling=True, inference=False, explanatory=False):
        super(ReferSegmDataset, self).__init__()
    ...
    def initialize_refer_segm_data(self, refer_segm_data, inference=False):
        ...
        dataset_dir = os.path.join(self.dataset_dir, "refer_seg")
        ...
        for dataset_name in self.refer_seg_ds_list:
            ...
            self.data2num[dataset_name] = len(refer_seg_ds['images'])
    ...
    def load_images(self, refer_api, images_ids_train, dataset_dir, dataset_name, inference=False):
        ...
        for item in loaded_images:
            ...
            if dataset_name == 'refclef':
                ...
                item["file_name"] = os.path.join(dataset_dir, "images/mscoco/images/train2014", item["file_name"])
            images.append(item)
        return images
    ...
    def create_conversations(self, labels, masks):
        questions = []
        answers = []
        new_masks = []
        ...
        for i, label in enumerate(labels):
            ...
            mask = masks[i]
            if mask.sum() == 0:
                answers.append("Sorry, I don't see any {class_name} in this image.".format(class_name=label.lower()))
            else: 
                new_masks.append(mask)
                answer_template = random.choice(self.answer_list)
                if self.explanatory:
                    answers.append(answer_template.replace('[SEG]', f"<p> {label.lower()} </p> [SEG]"))
                else:
        ...
        return questions, conversations, new_masks
    ...
    def __getitem__(self, idx):
        ...
        ori_image = self.oss_load_img_cv2(image_path)
        global_enc_img = self.global_enc_processor.preprocess(ori_image, return_tensors="pt")["pixel_values"][0]
        image = self.transform.apply_image(ori_image)
        ...
        questions, conversations, masks = self.create_conversations(selected_labels, masks)
        ...
        if len(masks) > 0:
            ...
            masks = None
            label = torch.ones(ori_image.shape[0], ori_image.shape[1]) * self.IGNORE_LABEL
...
class GReferCOCOValDataset(OSSDataset):
    CLASSES = ('object',)
    IMG_MEAN = torch.Tensor([123.675, 116.28, 103.53]).view(-1, 1, 1)
    IMG_STD = torch.Tensor([58.395, 57.12, 57.375]).view(-1, 1, 1)
    IMG_SIZE = 1024
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=500 * 8 * 2 * 10,
                 precision: str = "fp32", image_size: int = 224, num_classes_per_sample: int = 3,
                 refer_segm_data="grefcoco", validation=False, split='val',
                 random_sampling=True, inference=False, explanatory=False):
        super(GReferCOCOValDataset, self).__init__()
        self.epoch_samples = epoch_samples
        self.num_classes_per_sample = num_classes_per_sample
        ...
        self.image_dir = os.path.join(dataset_dir, "refer_seg/images/mscoco/images/train2014")
        self.refer_seg_data = self.initialize_refer_segm_data(refer_segm_data, inference)

        print(f"dataset {refer_segm_data} ({self.split} split) has {len(self.refer_seg_data['images'])} "
                f"images and {len(self.refer_seg_data['refs'])} referings.")

    def initialize_refer_segm_data(self, refer_segm_data, inference=False):
        assert refer_segm_data == "grefcoco"
        dataset_name = refer_segm_data

        dataset_dir = os.path.join(self.dataset_dir, "refer_seg")
        refer_api = G_REFER(dataset_dir, dataset_name, "unc")
        ref_ids = refer_api.getRefIds(split=self.split)
        images_ids = refer_api.getImgIds(ref_ids=ref_ids)
        refs = refer_api.loadRefs(ref_ids=ref_ids)
        
        images = {}
        for id in images_ids:
            images[id] = refer_api.loadImgs(id)[0]
    ...
    def __len__(self):
        return len(self.refer_seg_data['refs'])
    ...
    def create_conversations(self, labels, masks):
        assert len(labels) == len(masks) == 1

        label = labels[0].strip()
        question_template = random.choice(self.question_templates)
        question = question_template.format(class_name=label.lower())
        ...
        if mask.sum() == 0:
            answer = "Sorry, I don't see any {class_name} in this image.".format(class_name=label.lower())
        else: 
            new_masks.append(mask)
            answer_template = random.choice(self.answer_list)
            if self.explanatory:
                answer = answer_template.replace('[SEG]', f"<p> {label.lower()} </p> [SEG]")
            else:
        ...
        conv.append_message(conv.roles[1], answer)
        conversations.append(conv.get_prompt())
        return None, conversations, new_masks

    def __getitem__(self, idx):
        refs = self.refer_seg_data["refs"]
        images = self.refer_seg_data["images"]
        annotations = self.refer_seg_data["annotations"]
        
        ref_info = refs[idx]
        image_info = images[ref_info["image_id"]]

        sents = []
        ann_ids = []
        for sent in ref_info["sentences"]:
            text = sent["sent"]
            sents.append(text)
            ann_ids.append(ref_info["ann_id"])

        if len(sents) == 0:
            return self.__getitem__(random.randint(0, self.__len__() - 1))

        sampled_sents = [sents[0]]
        sampled_ann_ids = [ann_ids[0]]
        selected_labels = sampled_sents
