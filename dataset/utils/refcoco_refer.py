...
class REFER:
    def __init__(self, data_root, dataset="refcoco", splitBy="unc"):
        ...
        self.data["refs"] = pickle.loads(oss_get_file(ref_file).read())

        # load annotations from data/dataset/instances.json
        instances_file = osp.join(self.DATA_DIR, "instances.json")
        instances = json.load(oss_get_file(instances_file))
    ...
    def loadAnns(self, ann_ids=[]):
        ...
        elif type(ann_ids) == int or type(ann_ids) == str:  # replace `unicode` in Python2 with `str` in python3
