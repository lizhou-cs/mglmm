...
class RegionBaseDataset(OSSDataset):
    CLASSES = ('object',)
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=8000, precision="fp32",
        ...
        super(RegionBaseDataset, self).__init__()
    ...
    def _load_annotations(self, ann_file):
        ...
        img_ids = img_ids[0: 1000] if self.validation else img_ids
        data_infos = []
    ...
    def __getitem__(self, idx):
        ...
        ann_info = self.coco.loadAnns(self.coco.getAnnIds(imgIds=data_info['id']))
        ann = self._parse_annotations(data_info, ann_info)

        data_item = {
            "image_path": os.path.join(self.image_folder, data_info['file_name']),
