...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None, init_mm_projector=True):
        ...
        if init_mm_projector:
            modules = [nn.Linear(self.config.mm_hidden_size, self.config.hidden_size),
                        nn.GELU(),
                        nn.Linear(self.config.hidden_size, self.config.hidden_size)]
            self.mm_projector = nn.Sequential(*modules)

        if (pretrain_mm_mlp_adapter is not None) and (pretrain_mm_mlp_adapter != ""):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, past_key_values, labels, images, images_desc):
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            curr_full_input_ids = []
            # there is no image token in the current input_ids
